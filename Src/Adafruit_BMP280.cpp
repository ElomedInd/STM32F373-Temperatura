...
void Adafruit_BMP280::write8(byte reg, byte value) {
  //if (_cs == -1) {
    ...
    HAL_I2C_Master_Transmit(&hi2c1,_i2caddr,buf,2,-1);
    /*
...
uint8_t Adafruit_BMP280::read8(byte reg) {
  ...
  //if (_cs == -1) {
    HAL_I2C_Master_Transmit(&hi2c1,_i2caddr,&reg,1,-1);
    HAL_I2C_Master_Receive(&hi2c1,_i2caddr,&value,1,-1);
    /*
    _wire->beginTransmission((uint8_t)_i2caddr);
    _wire->write((uint8_t)reg);
    _wire->endTransmission();
...
uint16_t Adafruit_BMP280::read16(byte reg) {
  ...
  //if (_cs == -1) {
    HAL_I2C_Master_Transmit(&hi2c1,_i2caddr,&reg,1,-1);
    HAL_I2C_Master_Receive(&hi2c1,_i2caddr,(uint8_t*)buf,2,-1);
    value = (buf[0] << 8) | buf[1];
    /*
    _wire->beginTransmission((uint8_t)_i2caddr);
    _wire->write((uint8_t)reg);
...
uint32_t Adafruit_BMP280::read24(byte reg) {
  ...
  //if (_cs == -1) {
    ...
    HAL_I2C_Master_Transmit(&hi2c1,_i2caddr,&reg,1,-1);
    HAL_I2C_Master_Receive(&hi2c1,_i2caddr,(uint8_t*)buf,3,-1);
    value = buf[0];
    value <<= 8;
    value |= buf[1];
    value <<= 8;
...
bool Adafruit_BMP280_Temp::getEvent(sensors_event_t *event) {
  ...
  event->timestamp = HAL_GetTick();//millis();
