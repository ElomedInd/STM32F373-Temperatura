...
{
    uint8_t d[2];
    int ret = HAL_I2C_Mem_Read(&hi2c1, mlx90632_get_address(), register_address,
...
int32_t mlx90632_i2c_write(int16_t register_address, uint16_t value)
{
    ...
    d[0] = value >> 8;
    d[1] = value;
    if (HAL_I2C_Mem_Write(&hi2c1, mlx90632_get_address(), register_address,
...
{
    uint8_t d[2];
    if (HAL_I2C_Mem_Read(&hi2c1, mlx90632_get_address(), register_address + 1,
                         I2C_MEMADD_SIZE_16BIT, d, 2, 50) != HAL_OK)
        return -1;
    *value = d[0];
    *value = *value << 8;
...
static uint8_t mlx90632_address = 0;
void mlx90632_set_address(uint8_t address){
    mlx90632_address = address;
}

uint8_t mlx90632_get_address(){
    if(mlx90632_address == 0)
        return 0x3A << 1;
    else
        return mlx90632_address << 1;
}
